export const portfolioData = [
  {
    id: 1,
    title: 'Student Management System',
    description: 'A full-featured online shopping platform with secure payment integration, inventory management, and customer analytics.',
    image: '/sms.png',
    tags: ['web', 'design'],
    technologies: ['React', 'Node.js', 'MongoDB', 'Stripe'],
    link: 'https://alveeweb.netlify.app',
    github: 'https://github.com/AlveeHossain45',
    featured: true
  },
  {
    id: 2,
    title: 'Student Attendance System',
    description: 'An efficient system for tracking student attendance with real-time reporting and analytics.',
    image: '/sms4.png',
    tags: ['web', 'mobile'],
    technologies: ['Angular', 'Express', 'MySQL', 'Redis'],
    link: 'https://samisahil.github.io/attendance/',
    github: '#',
    featured: true
  },
  {
    id: 3,
    title: 'School Management System Basic',
    description: 'Mobile application for tracking workouts and nutrition with AI recommendations and social features.',
    image: '/sms3.png',
    tags: ['mobile', 'design'],
    technologies: ['React Native', 'Firebase', 'TensorFlow'],
    link: '#',
    github: '#',
    featured: true
  },
  {
    id: 4,
    title: 'AIC Inventory -- Management System',
    description: 'Analytics dashboard for business intelligence and data visualization with real-time reporting.',
    image: '/sms2.png',
    tags: ['web'],
    technologies: ['Vue.js', 'D3.js', 'Python', 'PostgreSQL'],
    link: 'https://samisahil.github.io/aiciinventoery/',
    github: '#',
    featured: false
  },
  {
    id: 5,
    title: 'Educational Platform',
    description: 'Interactive learning management system with video conferencing, assignments, and progress tracking.',
    image: 'https://images.unsplash.com/photo-1497633762265-9d179a990aa6?auto=format&fit=crop&q=80&w=870',
    tags: ['web', 'design'],
    technologies: ['React', 'Django', 'WebRTC', 'SQLite'],
    link: '#',
    github: '#',
    featured: false
  },
  {
    id: 6,
    title: 'Social Media App',
    description: 'Community-driven social platform with content sharing, messaging, and moderation features.',
    image: 'https://images.unsplash.com/photo-1516557070042-d968619ec4af?auto=format&fit=crop&q=80&w=870',
    tags: ['mobile'],
    technologies: ['Flutter', 'Firebase', 'Cloud Functions'],
    link: '#',
    github: '#',
    featured: false
  },
  {
    id: 7,
    title: 'Real Estate Listing',
    description: 'Property listing platform with advanced search filters, virtual tours, and agent management.',
    image: 'https://images.unsplash.com/photo-1582407947304-fd86f028f716?auto=format&fit=crop&q=80&w=783',
    tags: ['web', 'design'],
    technologies: ['Next.js', 'TailwindCSS', 'Mapbox', 'Strapi'],
    link: '#',
    github: '#',
    featured: true
  },
  {
    id: 8,
    title: 'Food Delivery Service',
    description: 'End-to-end food ordering and delivery platform with restaurant management and tracking.',
    image: 'https://images.unsplash.com/photo-1580959375944-abd7e991f972?auto=format&fit=crop&q=80&w=774',
    tags: ['web', 'mobile'],
    technologies: ['React', 'React Native', 'Node.js', 'MongoDB'],
    link: '#',
    github: '#',
    featured: false
  }
];

export default portfolioData;